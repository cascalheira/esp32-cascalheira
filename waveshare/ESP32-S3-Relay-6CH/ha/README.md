...
(number, minutes, 0 = off), Relay N safeguard tripped (binary sensor), Relay N max per day (number, minutes), Relay N on today (sensor), Relay N total on time (total_increasing sensor for the Statistics dashboard), Relay N daily limit reached (binary sensor), Mode (auto/manual/off), Exclusive mode (switch: only one relay on at a time), Relay N on when clock unknown (switch: the state held after a cold boot with no network and no time), Buzzer (switch), Rain hold (number, hours) + Rain hold until, Free heap / Lowest free heap (sensors), Last reset reason, Firmware (update entity with Install button), Last crash + Clear crash report,
