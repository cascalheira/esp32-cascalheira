...
| 5 ◐ OTA via HTTP pull + rollback ✅, task watchdog ✅ (2026-09-05); RSSI/uptime ✅; buzzer + LED pages pending | Ops | OTA via `EspOta` from an HTTP URL sent on `cmd`, rollback enabled, task watchdog, RSSI/uptime diagnostics, buzzer feedback, LED status pages. |
